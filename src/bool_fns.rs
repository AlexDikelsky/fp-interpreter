...
#[allow(dead_code)]
pub fn and<'a>(x: Option<AtomOrSeq>) -> Option<AtomOrSeq> {
    bool_fn(x, Box::new(|x, y| x && y))
}

#[allow(dead_code)]
pub fn or<'a>(x: Option<AtomOrSeq>) -> Option<AtomOrSeq> {
    bool_fn(x, Box::new(|x, y| x || y))
}

#[allow(dead_code)]
fn bool_fn<'a>(x: Option<AtomOrSeq>, f: Box<Fn(bool, bool) -> bool>) -> Option<AtomOrSeq> {
    match x {
        Some(x) => match x {
...
#[allow(dead_code)]
pub fn eq(x: Option<AtomOrSeq>) -> Option<AtomOrSeq> {
    match x {
        Some(x) => match x {
            Seq(s) => match (s.get(0), s.get(1), s.get(2)) {
                (Some(y), Some(z), None) => match (y, z) {
                    (Seq(ys), Seq(zs)) => {
                        zip_eq(ys.iter(), zs.iter())
                            .fold(Some(TRUE.clone()), |acc, (y_i, z_i)| {
